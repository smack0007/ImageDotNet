...
namespace CodeGenerator
{
    public static partial class Program
    {
        ...
        public static void Main(string[] args)
        {
            ...
            WriteIImage(libraryPath, pixels);

            WriteImageT(libraryPath, pixels);
        ...
        private static void WriteIImage(string libraryPath, string[] pixels)
        {
            var pixelMethods = BuildIImagePixelMethods(pixels);

            File.WriteAllText(Path.Combine(libraryPath, $"IImage.g.cs"),
$@"/// <auto-generated />
using System;
...
{{
    ...
    {{
{pixelMethods.ToString().TrimEnd()}        
    }}
}}");
        }

        private static StringBuilder BuildIImagePixelMethods(string[] pixels)
        {
            var methods = new StringBuilder(1024);

            foreach (var pixel in pixels)
            {
                methods.AppendLine($"{tab}{tab}bool Is{pixel}();");
                methods.AppendLine();

                methods.AppendLine($"{tab}{tab}Image<{pixel}> To{pixel}();");
                methods.AppendLine();
            }
        ...
        private static void WriteImageT(string libraryPath, string[] pixels)
        {
            var pixelMethods = BuildImageTPixelMethods(pixels);

            File.WriteAllText(Path.Combine(libraryPath, "Image{T}.g.cs"),
$@"/// <auto-generated />
using System;
...
{pixelMethods.ToString().TrimEnd()}        
        ...
        private static StringBuilder BuildImageTPixelMethods(string[] pixels)
        {
            var methods = new StringBuilder(1024);

            foreach (var pixel in pixels)
            {
                methods.AppendLine($"{tab}{tab}public bool Is{pixel}() => typeof(T) == typeof({pixel});");
                methods.AppendLine();

                methods.AppendLine($"{tab}{tab}public Image<{pixel}> To{pixel}() => new Image<{pixel}>(Width, Height, PixelHelper.Convert<T, {pixel}>(_pixels));");
                methods.AppendLine();
            }
...
{saveAndLoadTests.ToString().TrimEnd()}
        ...
        private static object BuildSaveAndLoadTests(string fileFormat, string[] pixels)
        {
            ...
            {
                var assertMethod = $@"
        private static void AssertSaveAndLoad(Image<{pixel}> expected)
        {{
            Image<{pixel}> actual = null;

            using (var memory = new MemoryStream())
            {{
                expected.SavePng(memory);
                memory.Position = 0;
                actual = Image.LoadPng(memory).To{pixel}();
            }}

            AssertEx.Equal(expected, actual);
        }}";

                output.AppendLine(assertMethod);
                output.AppendLine();
